...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('vezlo_knowledge_chunks', (table) => {
    ...
    table.specificType('embedding', 'vector(1536)').comment('OpenAI embeddings for semantic search');
  ...
  // Vector similarity index for semantic search (ivfflat for efficient nearest-neighbor search)
  await knex.schema.raw(`
    CREATE INDEX IF NOT EXISTS idx_vezlo_chunks_embedding 
    ON vezlo_knowledge_chunks USING ivfflat (embedding vector_cosine_ops)
