...
# Install only production dependencies + ts-node (needed to run TS migrations)
RUN npm ci --only=production && npm install ts-node && npm cache clean --force

# Copy built application (dist) and TypeScript migrations from builder
COPY --from=builder /app/dist ./dist
COPY --from=builder /app/src/migrations ./src/migrations
# Copy knexfile (both TS source and compiled JS used by runtime)
